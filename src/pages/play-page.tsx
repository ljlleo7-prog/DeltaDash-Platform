      ...
      <section className="grid gap-4 lg:grid-cols-3">
        {playCards[language].map((card) => (
          <article key={card.title} className="rounded-3xl border border-white/10 bg-white/5 p-5">
            ...
            <Link to={card.href} className="mt-5 inline-flex rounded-full border border-[var(--accent-hot)]/35 bg-[rgba(85,199,255,0.08)] px-5 py-3 text-sm font-medium text-[var(--text-main)] transition hover:bg-[rgba(255,77,90,0.16)]">
              {card.cta}
            </Link>
          </article>
        ))}
